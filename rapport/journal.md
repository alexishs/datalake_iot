...
- **Airflow d'OpenMetadata non utilisé (supprimable)** : le catalogage étant fait en *config-as-code* (CLI `metadata`, mode « ingestion externe » d'OpenMetadata), l'**ordonnanceur Airflow intégré d'OM** (conteneur `openmetadata-ingestion`) ne sert ici qu'à **héberger la CLI** — sa fonction de planification et de supervision via l'UI n'est pas exploitée. Il **pourrait être retiré** de la stack (exécution de la CLI dans un conteneur éphémère), ce qui l'allègerait. Conservé par souci de rester proche d'un déploiement OpenMetadata **standard** ; en production on le garderait pour **planifier des ré-ingestions récurrentes** (les métadonnées dérivent), ou l'on brancherait OM sur un **orchestrateur existant**.
